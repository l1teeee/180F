...
- Data persistence — changes are kept in the browser's local storage for the current day, survive a reload and sync across open tabs; nothing is ever sent to a server (ADR-022).
...
- **Data persists for the browser session and syncs across tabs.** Bookings, customers and settings you change are kept in local storage for the current day, so a reload or a second tab shows the same state. The dataset starts fresh on a new day, or immediately through **Reset demo data** in the Demo Mode badge or in Settings. Nothing is written to a database (ADR-022, master plan §24).
