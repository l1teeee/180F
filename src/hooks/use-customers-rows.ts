...
// roster, then selectCustomerStats resolves each remaining customer's per-row view model.
...
import { filterCustomers, selectCustomerStats } from '@/domain/selectors';
...
export function useCustomersRows(filters: CustomerFilters): CustomerWithStats[] {
  ...
  return useMemo(() => {
    ...
    return filtered.map((customer) => selectCustomerStats(customer, bookings, sessions, membershipPlans, demoToday));
