...
// - this component stays presentational rather than reading the catalog store itself.
...
export function CustomerStats({ stats, favoriteClassName }: CustomerStatsProps) {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
      <StatCard label="Classes this month" value={stats.classesThisMonth} icon={CalendarCheck} accent="purple" />
      ...
      <StatCard label="Attendance rate" value={stats.attendanceRate} unit="%" icon={TrendingUp} accent="green" />
      <StatCard label="No-shows" value={stats.noShows} icon={UserX} accent="pink" />
      <StatCard label="Favorite class" value={favoriteClassName ?? 'None yet'} icon={Heart} accent="yellow" />
    </div>
  );
}
